...
function doPost(e) {
  try {
    ...
    if (!action) {
      ...
      result = handlePedirCorrecciones(tokenFrom(data, e), data && data.id);
    } else if (action === 'panel/board/enviar-correcciones') {
      result = handleEnviarCorrecciones(tokenFrom(data, e), data && data.id, data && data.fileId, data && data.mensaje);
    ...
    } else if (action === 'panel/board/archivos') {
      ...
      result = handleBorrarProduccion(tokenFrom(data, e), data && data.id);
