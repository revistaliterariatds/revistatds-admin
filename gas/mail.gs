...
function sendAsignacion(editorEmail, produccion, docUrl) {
  var subject = 'Nueva producción asignada — ' + produccion.titulo + ' · ' + produccion.autor;
  var html = [
    '<div style="font-family:Lato,Arial,sans-serif;color:#1e1a17;background:#f0ece3;padding:28px;max-width:600px;margin:0 auto;border:1px solid #cec8bc;">',
    '  <h1 style="font-family:\'Playfair Display\',Georgia,serif;color:#1e1a17;font-weight:400;margin:0 0 12px;">Nueva producción asignada</h1>',
    '  <p style="font-size:16px;line-height:1.6;margin:0 0 20px;"><strong>' + escapeHtml(produccion.titulo) + '</strong> · ' + escapeHtml(produccion.autor) + '</p>',
...
function sendConsultaAutor(autorEmail, produccion, token, textoCorregido, pdfBlob) {
  var subject = getMailSubject('mail_subject_consulta', 'Tu producción está lista para aprobación — {{titulo}}', { titulo: produccion.titulo });
...
// Aviso al equipo cuando el autor pide modificar su producción (subió una nueva
// versión desde CONSULTA_AUTOR): editor asignado + COORDINADOR/WEBMASTER/SUPERVISOR.
function sendSolicitudModificacion(produccion) {
  var destinatarios = [];
  if (produccion.editor_asignado) destinatarios.push(String(produccion.editor_asignado));
  getEmailsByRoles(ROLES_NOTIF_GESTORES).forEach(function (e) {
    if (destinatarios.indexOf(e) < 0) destinatarios.push(e);
  });
  if (destinatarios.length === 0) return;
  var subject = 'El autor solicita modificar su producción — ' + produccion.titulo + ' · ' + produccion.autor;
  var html = [
    '<div style="font-family:Lato,Arial,sans-serif;color:#1e1a17;background:#f0ece3;padding:28px;max-width:600px;margin:0 auto;border:1px solid #cec8bc;">',
    '  <h1 style="font-family:\'Playfair Display\',Georgia,serif;color:#1e1a17;font-weight:400;margin:0 0 12px;">El autor quiere modificar su producción</h1>',
    '  <p style="font-size:16px;line-height:1.6;margin:0 0 12px;"><strong>' + escapeHtml(produccion.titulo) + '</strong> · ' + escapeHtml(produccion.autor) + '</p>',
    '  <p style="font-size:16px;line-height:1.6;margin:0 0 20px;">El autor subió una nueva versión de su producción y solicita que el equipo editorial se contacte con él para definir cómo proceder.</p>',
...
function sendLiberacion(editorEmail, produccion) {
  var subject = 'Fuiste liberado de — ' + produccion.titulo;
  var html = [
    '<div style="font-family:Lato,Arial,sans-serif;color:#1e1a17;background:#f0ece3;padding:24px;max-width:600px;margin:0 auto;border:1px solid #cec8bc;">',
    '  <p style="font-size:16px;line-height:1.6;margin:0;">Fuiste liberado de <strong>' + escapeHtml(produccion.titulo) + '</strong>.</p>',
    '  <p style="margin:16px 0 0;"><a href="' + PANEL_URL + '" style="color:#d95f1a;">Ver panel</a></p>',
    '</div>',
  ].join('');
  sendHtmlMail(editorEmail, subject, html);
